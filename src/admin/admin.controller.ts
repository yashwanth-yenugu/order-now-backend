...
import { ApiImplicitBody, ApiImplicitParam, ApiImplicitQuery } from '@nestjs/swagger';
...
export class AdminController {
  constructor( private adminService:AdminService){}
   ...
   async  getOrders(@Param() orderId){
        ...
        const restaurant = await Restaurants.findOne({RestaurantId:order.RestaurantId});
        const table = await Tables.findOne({TableId:order.TableId})
        const menu = await createQueryBuilder().select().from(Menu,'M')
    ...
    bookingInput.MenuId = bookingInput.MenuId.length > 0?bookingInput.MenuId:null;
    ...
    @ApiImplicitQuery({'name':'restaurantId', 'description': 'number','required': true})
    ...
    async login(@Body() input) {
      try {
        ...
        const otpRes = await this.sendOTP({ phoneNumber: '+91' + input.Mobile.slice(-10) });
